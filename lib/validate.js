...
function runValidation(targetRoot, pkgRoot, rawOptions = {}) {
  ...
  // repo-docs scope validates only README.md / docs/ — memory-bank need not exist.
  if (opts.scope !== 'repo-docs' && !fs.existsSync(memoryBankDir)) {
...
// Returns an array of { watchPath, label, isFile } objects for fs.watch,
// based on the validation scope. Used by the CLI watch mode.
function getWatchRoots(targetRoot, scope) {
  const roots = [];
  const normalizedScope = VALIDATION_SCOPES.has(scope) ? scope : 'all';

  if (normalizedScope !== 'repo-docs') {
    roots.push({
      watchPath: path.join(targetRoot, 'memory-bank'),
      label: 'memory-bank/',
      isFile: false
    });
  }
  ...
  if (normalizedScope !== 'memory') {
    ...
    if (fs.existsSync(docsDir)) {
      roots.push({ watchPath: docsDir, label: 'docs/', isFile: false });
    }
    if (fs.existsSync(readmePath)) {
      roots.push({ watchPath: readmePath, label: 'README.md', isFile: true });
    }
  }
