...
function validateMarkdownFile(fullPath, targetRoot, schemas, opts, results, fileRecords) {
  ...
  if (!schema) {
    ...
    validateSchemaValue(fields, schema, 'frontmatter', errors, warnings, opts);
  }
  ...
  warnings.forEach((message) => addResult(results, opts.strict ? 'errors' : 'warnings', relPath, message, 'schema'));
}
...
function checkMarkdownLinks(targetRoot, fileRecords, results) {
  const markdownRoots = ['memory-bank', 'docs', 'README.md']
...
function runValidation(targetRoot, pkgRoot, rawOptions = {}) {
  ...
  const results = { profile: opts.profile, errors: [], warnings: [], ok: [], skipped: [] };
  ...
  checkMarkdownLinks(targetRoot, fileRecords, results);
