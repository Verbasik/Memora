  ...
  for (const hookScript of manifest.doctor.hookConfigFiles) {
    ...
    for (const ref of refs) {
      ...
      if (fs.existsSync(ref)) {
        addResult(results, 'ok', 'hooks', `Hook path is valid: ${relPath} (from ${hookScript})`);
      } else {
        addResult(results, 'errors', 'hooks', `Broken hook path reference: ${relPath} (from ${hookScript})`);
      }
    }
  }
}
...
function checkCriticalPlaceholders(targetRoot, manifest, results, placeholderAllowlist) {
  for (const relativePath of manifest.doctor.criticalFiles) {
    ...
    if (placeholderAllowlist && placeholderAllowlist.has(relativePath)) {
      addResult(results, 'ok', 'placeholders', `Intentional placeholder (source repo): ${relativePath}`);
      continue;
    }
...
function runDoctor(targetRoot) {
  ...
  const placeholderAllowlist = loadSourcePolicy(targetRoot);
  ...
  checkCriticalPlaceholders(targetRoot, manifest, results, placeholderAllowlist);
