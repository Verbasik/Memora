...
function _buildSessionStartOutput(additionalContext) {
  ...
  // Codex SessionStart output: plain text on stdout (safest per official docs 2026-04-17).
  // Docs also accept { hookSpecificOutput: { additionalContext } } JSON, but plain text
  // avoids the prior snake_case mistake ({ additional_context }) and matches UserPromptSubmit.
...
function handleUserPromptSubmit(payload = {}, deps = {}) {
  ...
  if (sessionId && prompt) {
    rt.recordTurnUserMessage(sessionId, { content: prompt, source: 'codex' });
  }
  ...
  // Codex UserPromptSubmit output: plain text on stdout.
...
/**
 * Handle Codex Stop — turn-level checkpoint.
 *
 * Codex Stop payload (per official docs 2026-04-17):
 *   session_id            — session identifier
 *   turn_id               — current turn identifier
 *   last_assistant_message — the assistant's final message text (guaranteed)
 *   transcript_path       — path to native transcript file (if available)
 *   stop_hook_active      — guard flag
 *
 * Strategy:
 *   1. Append last_assistant_message directly (fast path, always available).
 *   2. If transcript_path is present, also sync to catch any missed messages.
 */
function handleStop(payload = {}, deps = {}) {
  ...
  // Fast path: Codex guarantees last_assistant_message on Stop.
  const lastMsg = payload.last_assistant_message || null;
  if (lastMsg && typeof lastMsg === 'string' && lastMsg.trim()) {
    rt.recordTurnAssistantMessage(sessionId, {
      content: lastMsg,
      source:  'codex',
    });
  }

  // Repair path: sync from native transcript if available.
  const transcriptPath = payload.transcript_path || null;
  if (transcriptPath) {
    nativeSync.syncFromPath(sessionId, transcriptPath, { rt, source: 'codex' });
  }
...
/**
 * Ensure `codex_hooks` feature is enabled in the Codex global config.
 *
 * @param {object} [opts]
 * @param {string} [opts.configPath]   - Override path to ~/.codex/config.toml (for testing)
 * @param {object} [opts.fsModule]     - Injectable fs (for testing)
 * @param {function} [opts.execFn]     - Injectable exec function (for testing)
 * @returns {{ status: 'already'|'enabled'|'skipped'|'unavailable', reason?: string }}
 */
function ensureCodexHooksEnabled(opts = {}) {
  const fsModule    = opts.fsModule || fs;
  const execFn      = opts.execFn   || _defaultExec;
  ...
  try {
    ...
    execFn('codex', ['features', 'enable', 'codex_hooks']);
