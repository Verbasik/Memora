...
function scanContextContent(content, filename) {
  ...
  // Invisible Unicode check — normalise to 'invisible_unicode' (consistent with
  // scanMemoryContent); the specific char code is captured in the findings detail
  // and surfaced via the reason/sanitized strings.
  for (const char of INVISIBLE_CHARS) {
    if (content.includes(char)) {
      findings.push('invisible_unicode');
      break; // one invisible-unicode finding is sufficient
    }
  }
